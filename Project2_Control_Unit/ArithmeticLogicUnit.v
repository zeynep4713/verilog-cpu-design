...
module ArithmeticLogicUnit(A,B,FunSel,ALUOut,FlagsOut,WF,Clock);
            ...
            temp_O = 0;
               case(FunSel)
                  ...
                  begin
                   ...
                   5'b10100:  // A + B
                   begin
                        {temp, ALUOut} = A + B; //carry stored in temp
                        if((A[15] == B[15]) && (ALUOut[15]!== A[15]))begin
                            temp_O = 1; //overflow occurs:  pos + pos -> neg or neg + neg -> pos  !!CHANGED
                        end
                   end

                   5'b10101:  // A + B + Carry
                   begin
                        {temp, ALUOut} = A + B + C;  //carry stored in temp
                        if((A[15] == B[15]) && (ALUOut[15]!== A[15]))begin
                            temp_O = 1; //overflow occurs:  pos + pos -> neg or neg + neg -> pos  !!CHANGED
                        end
                   end
            ...
            always @(posedge Clock) begin
                if(WF == 1)begin
                Z = (ALUOut == 16'b0); // zero flag
                C = temp;
                
                //added part for N (negative number) flag
                if(FunSel[4]==0) //8-bit
                begin
                    N = (ALUOut[7] == 1);
                end
                else if(FunSel[4]==1) //16-bit
                begin
                    N = (ALUOut[15] == 1); 
                end
                O = temp_O;
